...
/*!
	...
	@constant		kAudioUnitSubType_VoiceProcessingIO
						- iPhone only. Available only with iPhone 3.0 or greater
					This audio unit works in a very similar way to the Remote I/O, but it adds the ability to do signal processing on
					the incoming audio (taking any of the audio that is played from the device at a given time from the incoming audio)
*/
enum {
...
*/
extern OSStatus
AudioUnitInitialize(				AudioUnit				inUnit)						__OSX_AVAILABLE_STARTING(__MAC_10_0,__IPHONE_2_0);
...
*/
extern OSStatus
AudioUnitUninitialize(				AudioUnit				inUnit)						__OSX_AVAILABLE_STARTING(__MAC_10_0,__IPHONE_2_0);
