...
@implementation ATV2Appliance

@synthesize topShelfController = _topShelfController;

+ (NSArray*) applianceCategories {
	
	NSMutableArray* categoryList = [[NSMutableArray alloc] initWithCapacity:6];
...
//zet onderstaande uit, dan komt dit menu niet in het hoofd menu
    [categoryList addObject:[BRApplianceCategory categoryWithName:LOG_CATEGORY_NAME
													   identifier:LOG_ID
												   preferredOrder:LOG_PREFERRED_ORDER]];
...
- (id)init {
    ...
    if (self) {
		...
		_applianceCategories	= [[ATV2Appliance applianceCategories] retain];
    }
...
- (id) controllerForIdentifier:(id)identifier args:(id)args
{
    ...
    else if ([identifier isEqualToString:LOG_ID]) {
		controller	= [[[LogMainMenu alloc] init] autorelease];
	}
